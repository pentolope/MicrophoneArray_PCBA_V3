...
Research machinery, kept beside the search harness that uses it. The generic
toolkit has no cross-run artifact cache to keep fresh: its derived results are
produced and consumed inside one run, and the identity a release needs is
Git's. A candidate search is different - it persists pools, decisions and
comparison reports across runs and must know when one has gone stale.
