...
from research import progression                       # noqa: E402
from research.progression import ProgressionError      # noqa: E402
...
class ConnectivityIsJudgedOverTheBoardRequiredSet(unittest.TestCase):

    def test_an_incomplete_board_is_never_fully_connected(self):
        """79/83 board-wide is NOT fully connected, and progression
        stops at board-connectivity."""
    ...
    def test_no_other_net_set_can_be_reported_beside_it(self):
        """The input carries one connectivity set and refuses another;
        a second set is how a partial one starts standing in for it."""
        with self.assertRaises(ProgressionError) as caught:
            progression.assess(dict(
                _record(), other_connectivity={"complete": 46,
                                               "total": 46}))
        self.assertIn("other_connectivity", str(caught.exception))

    def test_rank_prefers_a_complete_board_over_later_metrics(self):
        """A board-complete candidate outranks an incomplete one
        whatever its later metrics."""
        complete = progression.assess(_record(
            electrical_evidence={"usable_results": 0},
            optimization={"copper_mm": 9999.0}))
        incomplete = progression.assess(_record(
            board_required_connectivity={"complete": 79,
                                         "total": 83},
